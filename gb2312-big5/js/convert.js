...
function parseDomainList(list_of_regex) {
...
function matchesPattern(value, pattern) {
  ...
  try {
    ...
    // Ignore invalid regular expressions and rely on substring matching above.
    return false;
  }
}

function checkList(url, list_of_regex) {
  const patterns = parseDomainList(list_of_regex);
  if (patterns.length === 0) {
    return false;
  }
  const domain = getDomain(url);
  return patterns.some(
    (pattern) => matchesPattern(domain, pattern) || matchesPattern(url, pattern)
  );
}

function shouldTranslatePage(url, whitelist, blacklist) {
  if (checkList(url, blacklist)) {
    return false;
  }
  const whitelistPatterns = parseDomainList(whitelist);
  return whitelistPatterns.length === 0 || checkList(url, whitelist);
}

function runContentScript() {
  (async function () {
    const [setting, dialect, wlist, blist] = await Promise.all([
      chrome.storage.sync.get('setting'),
      chrome.storage.sync.get('dialect'),
      chrome.storage.sync.get('wlist'),
      chrome.storage.sync.get('blist')
    ]);

    async function work(setting, dialect, wlist, blist) {
      const url = window.location.toString();
      if (shouldTranslatePage(url, wlist, blist)) {
        translateBody(document.body, setting, dialect);
      } else {
        console.log(`${url} skipped by whitelist/blacklist`);
      }
    }

    if (setting && dialect && wlist && blist) {
      work(setting.setting, dialect.dialect, wlist.wlist, blist.blist);
      setTimeout(() => work(setting.setting, dialect.dialect, wlist.wlist, blist.blist), 2000);
      setTimeout(() => work(setting.setting, dialect.dialect, wlist.wlist, blist.blist), 3500);
      setTimeout(() => work(setting.setting, dialect.dialect, wlist.wlist, blist.blist), 5000);
    }
  })();
}

// Run automatically only inside the extension (content-script) context.
if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
  runContentScript();
}

// Export pure helpers for unit testing under Node/Jest (ignored in the browser).
